...
raw_dat <- read.csv(
  file.path(
    work_directory,
    "data",
    "figure_8_edible_projections.csv" #was figure 7 now figure 8
  ),
  header=TRUE
)
...
legend_object <- get_legend(edible_proj_plot)
edible_proj_plot <- edible_proj_plot + theme(legend.position = "none")

edible_proj_plot_clean <- ggdraw() +
  draw_plot(edible_proj_plot) +
  draw_plot(legend_object,x=0.25,y=-.33)

ggsave(
  edible_proj_plot_clean,
  filename = file.path(
    zach_wk_dir,
    "figures",
    "figure_8_edible_proj.pdf"
  ),
  width = 11.5,
  height = 7.39
